                            ...
                            <div class="flex items-center justify-center gap-1">
                                ...
                                <button 
                                    @if($user->username !== 'admin')
                                        @click="deleteId = {{ $user->id_user }}; deleteName = 'User ' + {{ \Illuminate\Support\Js::from($user->nama_lengkap) }}; showDeleteModal = true"
                                    @endif
                                    {{ $user->username === 'admin' ? 'disabled' : '' }}
                                    class="p-2 {{ $user->username === 'admin' ? 'text-[#334155] cursor-not-allowed opacity-50' : 'text-rose-400 hover:bg-rose-500/10' }} rounded-lg transition-colors" 
                                    title="{{ $user->username === 'admin' ? 'Admin utama tidak dapat dihapus' : 'Hapus' }}">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
